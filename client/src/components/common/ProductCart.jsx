...
export default function ProductCard({ product, onOrder }) {
  const { _id, name, price, images, description, rating, totalReviews, entrepreneur, stock, isAvailable } = product
      ...
      <Link to={`/marketplace/${_id}`} className="relative h-44 overflow-hidden bg-cream-dark block">
        ...
        {!available && (
          <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
            <span className="bg-white text-xs font-semibold px-3 py-1 rounded-full text-gray-700">Out of Stock</span>
          </div>
        )}
      </Link>
      <div className="p-4 flex flex-col flex-1">
        <Link to={`/marketplace/${_id}`} className="font-semibold text-charcoal text-sm leading-snug group-hover:text-saffron transition-colors line-clamp-1">{name}</Link>
        <p className="text-xs text-gray-400 mt-0.5 mb-1.5">by {entrepreneur?.user?.name || 'Artisan'}</p>
        <p className="text-xs text-gray-500 line-clamp-2 flex-1 leading-relaxed">{description}</p>
